...
#set document(title: [Regression analysis with fixest])
...
#link("https://lrberge.github.io/fixest")[*fixest*] (by Laurent Bergé) is a package
...
The #link("https://lrberge.github.io/fixest/index.html")[*fixest*] package contains a highly flexible set of functions that allow you to estimate a large set of regression models. While the package obviously doesn't cover _every_ model out there, there is a non-negligible subset of Stata users for whom every model they've ever needed to estimate is covered by *fixest*.
...
*fixest* offers all of this in addition to being #link("https://lrberge.github.io/fixest/#benchmarking")[_very_ fast]. If you felt a speed boost going from Stata's `xtreg` to `reghdfe,` get ready for another significant improvement when moving to *fixest*.
...
#link("https://lrberge.github.io/fixest/articles/multiple_estimations.html")[multi-model]
...
Multi-model objects can also be sent directly to #link("#presentation")[presentation]
...
#link("https://grantmcdermott.com/posts/better-way-adjust-ses/")[several benefits],
...
Aside: You should be using `, vce(hc2)` for smaller samples... but you
#link("http://datacolada.org/99")[knew that], right?
...
(#link("https://lrberge.github.io/fixest/articles/exporting_tables.html")[1],
#link("https://lrberge.github.io/fixest/articles/etable_new_features.html")[2]). Below we highlight a few unique features that don't have direct Stata
