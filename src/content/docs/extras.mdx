...
package. The **arrow** package provides a really fast and memory efficient data 
storage format (`.parquet`). The **duckdb** package provides a really fast and 
memory efficient database system. For example, imagine you have a 10M row 
dataset with hundreds of variables. You want to take group-averages of a few 
varaibles. Arrow/duckdb will do this *astonigshly* fast by not having to load 
an entire CSV into memory.

For this example, we will create a small dataset using the following code:
```r
data(airquality)
...
For example using a massive NYC Taxi dataset, see https://arrow.apache.org/docs/2.0/r/articles/dataset.html. The file is 37 gigabytes (bigger than most people's RAM) and arrow can do a `group_by()` and `summarize()` in a few seconds on a standard macbook. 

#### Import/export data with arrow

```r
airquality = arrow::open_dataset("airquality")
...
There is also the `.parquet` file format that we would use in place of `.csv` files. The parquet version will be much smaller in size and load/save faster.

```r
arrow::write_parquet(airquality, "airquality/data.parquet")
arrow::read_parquet(airquality, "airquality/data.parquet")
```
...
mean_ozone: double
mean_wind: double
n_days_wind_gt10: int64

See $.data for the source Arrow object
```

Instead of loading the output, `arrow` has "prepared" a set of commands that will need to be triggered with the `collect()` function. This allows arrow to do really efficient operations on very large data. Note that once you `collect()`, the result returned is a standard R data.frame, so do so carefully.
...
```text
# A tibble: 5 × 4
  Month mean_ozone mean_wind n_days_wind_gt10
  <int>      <dbl>     <dbl>            <int>
1     5       20.8      14.0               18
2     6       32.2      13.3               14
3     8       33        12                 12
...
```r
airquality_db = airquality |> to_duckdb()
```
```text
...
# Database: DuckDB 0.8.1 [root@Darwin 22.3.0:R 4.3.0/:memory:]
 ...
 8    19      99  13.8    59     8     5
 9     8      19  20.1    61     9     5
10    NA     194   8.6    69    10     5
# ℹ more rows
# ℹ Use `print(n = ...)` to see more rows
```

The standard dplyr functions work on duckdb datasets as well and requires the `collect()` function as well for execution. One nice thing `duckdb`/`arrow` do is the `explain()` function which explains what is happening under the hood for those curious. 
